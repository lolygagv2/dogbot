...
def get_led_controller():
    """Get LED controller from main LED service (single instance, no duplicates)"""
    ...
    if _led_controller is None:
        try:
            ...
            elif not led_service.led_initialized:
                # Try init (SPI-based, should always work)
                led_service.initialize()
                if led_service.led:
                    _led_controller = led_service.led
        except Exception as e:
            logger.warning(f"LED controller not available: {e}")
...
async def force_coaching_trick(trick: str, dog_id: str = None, dog_name: str = None,
                               audio_pre_played: bool = False, replace: bool = False):
    ...
    Query params:
        ...
        replace: cancel-and-replace — hard-cancel any in-progress session so this
            trick starts immediately instead of after the current one finishes.
    """
    engine = get_coaching_engine()
    result = engine.set_forced_trick(trick, dog_id=dog_id, dog_name=dog_name,
                                     audio_pre_played=audio_pre_played, replace=replace)
    return result
...
async def websocket_local(websocket: WebSocket):
    """Local mode WebSocket — same handler as /ws.

    No separate code path. Same commands, same WebRTC, same everything.
    """
    ws_server = get_websocket_server()
    await ws_server.handle_websocket(websocket)
