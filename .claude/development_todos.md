...
- [x] **Power-button NETWORK CANCEL — LIVE VERIFIED 2026-09-25 (tb5, Morgan)** — 5 s hold → WIMZ-5220 AP, phone joined, app Local Mode reached the robot. First attempt exposed the polkit bug (unprivileged nmcli writes denied from treatbot.service since ≥09-20; fixed 2219431 via sudo). Remaining: tb1/tb2/tb4 on site, Morgan updates manually: `cd /home/morgan/dogbot && git pull --ff-only && sudo bash scripts/ota/manual-update.sh` (installs watcher, restarts, verifies OTA layout intact).
